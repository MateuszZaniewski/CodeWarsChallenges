...
function step(g, m, n) {
    ...
    const primeNumbersArray = findPrimes(m, n)

    for(let i = 0; i < primeNumbersArray.length; i++){
        for(let k = 0; k < primeNumbersArray.length; k++){
            if(primeNumbersArray[i] - primeNumbersArray[k] == g){
                return [primeNumbersArray[k],primeNumbersArray[i]]
            }
        }
    }
